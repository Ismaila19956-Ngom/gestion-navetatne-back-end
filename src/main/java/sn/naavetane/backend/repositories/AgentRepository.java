...
import sn.naavetane.backend.models.AgentCountByDirectionDTO;
import sn.naavetane.backend.models.RetraiteProjectionDTO;
...
import sn.naavetane.backend.entities.QAgentEntity;
...
public interface AgentRepository extends JpaRepository<AgentEntity, Long>, QuerydslPredicateExecutor<AgentEntity> {
    ...
    @Query("SELECT new sn.naavetane.backend.models.AgentCountByDirectionDTO(d.libelle, COUNT(a)) " +
            "FROM AgentEntity a " +
            "JOIN a.direction d " +
            "GROUP BY d.libelle")
    ...
    @Query("""
                    SELECT new sn.naavetane.backend.models.RetraiteProjectionDTO(
