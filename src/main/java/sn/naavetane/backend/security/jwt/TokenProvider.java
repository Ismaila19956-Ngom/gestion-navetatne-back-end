package sn.naavetane.backend.security.jwt;

import sn.naavetane.backend.security.CustomUserDetails;
...
import sn.naavetane.backend.configurations.SecurityMetersConfiguration;
import sn.naavetane.backend.exceptions.UserDisabledException;
import sn.naavetane.backend.properties.JwtProperties;
import sn.naavetane.backend.repositories.UserRepository;
import sn.naavetane.backend.utils.HttpRequestResponseUtils;
...
//                }
        ...
        String profile = userRepository.findByLogin(authentication.getName())
                .map(userEntity -> userEntity.getProfile() != null ? userEntity.getProfile().getCode() : "")
                .orElse("");
        long now = (new Date()).getTime();
        Date validity;
        if (rememberMe) {
    ...
    public Authentication getAuthentication(String token) {
        ...
        if (Objects.isNull(userAgent) || !userAgent.equals(HttpRequestResponseUtils.getUserAgent())) {
//            throw new UsernameNotFoundException("Token not valid");
//            Désactivé temporairement pour le développement (problème d'accès mobile)
