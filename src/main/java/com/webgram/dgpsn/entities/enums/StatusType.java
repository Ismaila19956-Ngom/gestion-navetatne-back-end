...
public enum StatusType {
    ...
    public static StatusType fromValue(Object statusType) {
        if (statusType instanceof Map) {
            ...
            if (mapStatusType.containsKey("name")) {
                return StatusType.valueOf(mapStatusType.get("name").toString());
            }
        }
        if (statusType instanceof String) {
            return StatusType.valueOf(statusType.toString());
        }
        throw new IllegalArgumentException(MessageFormat.format("{0} not found with the value: {1} in [{2}]", StatusType.class, statusType, values()));
    }
